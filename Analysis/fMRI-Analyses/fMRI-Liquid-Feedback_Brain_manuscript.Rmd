...
library(dplyr)        # data manipulation R package
library(tidyr)        # data manipulation R package
#library(stringr)
#library(broom.mixed)
library(ggplot2)
library(ggpmisc)
...
library(mediation)
library(formattable)
source("../../Analysis/helper-functions/summarySEwithin2.R")
```
...
#data.path.betas<-here("Data/fMRI/SubjectData_Consolidated/")
data.path.betas<-"../../Data/fMRI/SubjectData_Consolidated/"

# behavioral data
#data.path.behavior<-here("/Data/Behavioral/Trimmed/subject_data.txt")
#runkey.path<-here("/Data/Behavioral/Trimmed/runkey.csv")
...
# output directories
#out.path<-here("/Analysis/fMRI-Analyses/")
#figure.path<-here("Analysis/fMRI-Analyses/Figures")
...
data.behavior <- tbl_df(read.delim(data.path.behavior))
data.runkey <- tbl_df(read.csv(runkey.path))
data.selfreport<-tbl_df(read.csv(selfreport.path))
...
## Summarising Bundled Beta Values in dorsal anterior cingulate cortex 
...
# Behavioral Analyses: Motivational Incentive Effects: Cognitive Task Performance and Self Report Measures
## Binomial Analyses
```{r}
...
m.RR.complex<-lme4::lmer(formula = meanRR ~ moneyCode * liqCode + (1+moneyCode+liqCode|subID), 
                      data = data.dACC.sum)

tab_model(m.RR,m.RR.JvN,m.RR.SvN,
          title = "Reward Rate by Experimental Factors",
          dv.labels = c("Reward Rate (Simple)","Juice v. Neutral","Saltwater v. Neutral"),
          pred.labels = c("Intercept","Money", "Liquid"),
          show.re.var = TRUE, show.stat = TRUE, show.icc = FALSE)

tab_model(m.RR,m.RR.complex,
          title = "Reward Rate by Experimental Factors",
          dv.labels = c("Reward Rate (Simple)","Reward Rate (Complex)"),
          pred.labels = c("Intercept","Money", "Liquid"),
          show.re.var = TRUE, show.stat = TRUE, show.icc = FALSE)
```
...
m.ACC<-lme4::lmer(formula = meanACC ~ moneyCode * liqCode + (1|subID), 
                      data = data.dACC.sum)
m.ACC.JvS<-lme4::lmer(formula = meanACC ~ moneyCode * liqCode + (1|subID), 
                      data = data.dACC.sum,subset = Liquid!="Neutral")
tab_model(m.RT, m.ACC,m.ACC.JvS,
          show.re.var = TRUE, show.stat = TRUE, show.icc = FALSE)
```
...
## Multiple Regression Between Reward Rate and Motivation Ratings
```{r}
m.motive<-lme4::lmer(formula = motive_rating ~ moneyCode + liqCode + (1+moneyCode+liqCode|subID), 
                      data = data.dACC.sum)

m.RR.motive<-lme4::lmer(formula = meanRR ~ moneyCode + liqCode + motive_rating + (1+moneyCode+liqCode+motive_rating|subID), 
                      data = data.dACC.sum)

tab_model(m.motive,m.RR.motive,
          title = "Hierarchical Regression with Motivation Ratings",
          dv.labels = c("Motivation Ratings","Reward Rate"),
          pred.labels = c("Intercept","Money", "Liquid","Motivation Ratings"),
...
# tmp.x<-coef(m.RR.motive)$subID[1]
# tmp.y<-coef(m.RR.motive)$subID[4]
# plot(x = tmp.x$`(Intercept)`, y = tmp.y$motive_rating)
```
...
#p.RR.motive + guides(color=guide_legend("my awesome title"))
...
p.figure1_sup<-cowplot::plot_grid(p.RT.mot9, p.ACC.mot9,
                              label_size = 16, ncol = 2, rel_widths = c(1,1.3))
p.figure1_sup
ggsave(filename = "Supplemental_Figure1.eps", plot = p.figure1_sup, 
       device="eps", dpi = 300,
       path = figure.path, width = 7, height = 3, scale = 1.2)
```

# ROI Analyses: Dorsal Anterior Cingulate Cortex (dACC)
...
m.dACC.RR.bundle<-lme4::lmer(formula = value.bundle.mean ~ moneyCode + liqCode + (1|subID), 
                      data = data.dACC.sum)
m.dACC.RR.bundle2<-lme4::lmer(formula = value.bundle.mean ~ moneyCode + liqCode + (1|subID), 
                      data = data.dACC.sum)
#tab_model(m.dACC.RR.bundle2)
```
...
## Figure 2: Plotting Blocked (Liquid) Effects & Cue (Money) Effects
```{r}
...
## Figure 2: Plotting Bundled Effects
```{r}
...
## Draw Tentzero function of trialwise beta estimates for Figure 2
```{r}
#'dACC' parcel ids from the incentive integration analyses
parcels.dACC<-c(107,110,108,311,312) 
parcels.vmpfc<-c(168, 169, 173, 174, 379, 380, 381, 382)
...
## Bundled Beta Values by Reward Rate in dorsal anterior cingulate cortex (Scatterplot)
```{r dACC-ROI, warning=FALSE, include=FALSE}
# # Scatterplot of dACC bundled betas by reward rate
# p.dACC<-ggplot(data = data.dACC.sum, aes(x = meanRR.normed, y = value.bundle.mean,
#                                          color = Reward, shape = Reward)) +
#   geom_point(size = 2) +
#   geom_smooth(method="lm", se = FALSE) +
#   facet_grid(.~Liquid) +
#   xlab("Reward Rate") + ylab("Dorsal ACC Beta Estimates") +
#   #coord_cartesian(xlim = c(-.2,.2),ylim=c(-.6,.6)) +
...
# p.dACC
# # # 
# # Scatterplot of dACC bundled betas by motivation ratings
# p.dACC.motive<-ggplot(data = data.dACC.sum, aes(x = motive_rating.centered, y = value.bundle.mean,
#                                                 color = Reward, shape = Reward)) +
#   geom_point(size = 2) +
#   geom_smooth(method="lm", se = FALSE) +
#   facet_grid(.~Liquid) +
#   xlab("Self Report Motivation Ratings") + ylab("Dorsal ACC Beta Estimates") +
#   #coord_cartesian(xlim = c(-.5,.3),ylim=c(-.6,.6)) +
#   #scale_y_continuous(breaks = c(1:7)) +
#   #scale_fill_discrete(name="Monetary Reward") +
#   scale_color_brewer(palette="Set1") +
#   theme_gray() +
...
# 
# # Save scatterplots
# ggsave(filename = "dACC_betas-rewardrate.png", plot = p.dACC, 
#        path = figure.path, device = "png", width = 8, height = 4)
# ggsave(filename = "dACC_betas-motiveratings.png", plot = p.dACC.motive, 
#        path = figure.path, device = "png", width = 8, height = 4)
# # Combined Figure 11
# p.figure11<-cowplot::plot_grid(p.dACC, p.dACC.motive,
#                               labels = c("11a","11b"), 
#                               rel_widths = c(1, 1),
#                               rel_heights = c(1.15, 1),
#                               align="v", nrow = 2)
# p.figure11
# ggsave(filename = "Figure11.eps", plot = p.figure11, 
#        device="eps", dpi = 300,
#        path = figure.path, width = 10, height = 8, scale = 1)
```


## Reward Rate predicted by Bundled Betas
## Brain-Behavior Associations in dACC (Mixed Effects Model)
```{r}
...
# m.dACC.RR2<-lme4::lmer(formula = meanRR ~ mot9Code + value.bundle.mean + (1+mot9Code|subID), 
#                       data = data.dACC.sum)
# tab_model(m.dACC.RR,m.dACC.RR2,
#           show.re.var = FALSE, show.stat = TRUE, show.icc = FALSE)
```
...
# Within-Subjects Mediation Analyses
x = dACC /newline
...
```{r}
# m.dACC.step1<-lme4::lmer(formula = meanRR ~ moneyCode + liqCode + (1|subID), 
#                          data = data.dACC.sum)
...
m.dACC.step3<-lme4::lmer(formula = meanRR ~ value.bundle.mean + motive_rating + (1|subID), 
                         data = data.dACC.sum)
tab_model(m.dACC.step3,
          title = "Mediated Reward Rate Effects",
          dv.labels = "Reward Rate",
          pred.labels = c("Intercept", "dACC Bundled Betas", "Motivation Ratings"),
          show.re.var = FALSE, show.stat = TRUE, show.icc = FALSE)
```
...
# Running the multilevel mediation on raw data
set.seed(200)
# dACC.mediation.results = mlm(d = data.dACC.mediation, id = "subID",
#                        x = "value.bundle.mean", m = "motive_rating", y = "meanRR",
#                        iter = 10000)
dACC.mediation.results = mlm(d = data.dACC.mediation, id = "subID",
                       x = "value.bundle.mean_cw", m = "motive_rating_cw", y = "meanRR_cw",
                       iter = 10000)

# if we want to run this mean centered
...
# Between-Subjects mediation analysis using mediate from mediation package
```{r}
fit.totaleffect<-lm(formula = meanRR.normed ~ value.bundle.mean.normed, data = data.dACC.sum.sub)
summary(fit.totaleffect)

fit.mediator<-lm(formula = motive_rating.normed ~ value.bundle.mean.normed, data = data.dACC.sum.sub)
summary(fit.mediator)

fit.dv<-lm(formula = meanRR.normed ~ value.bundle.mean.normed + motive_rating.normed, data = data.dACC.sum.sub)
summary(fit.dv)
  
results=mediate(model.m = fit.mediator, model.y = fit.dv,
                treat = "value.bundle.mean", mediator = "motive_rating")
summary(results)
...
## dACC Activation by Subject (Scatterplot)
...
ggsave(filename = "Figure3_dACC_bysubject.png", plot = p.dACC.bysub, 
        path = figure.path, device = "png", width = 10, height = 4.5)
# ggsave(filename = "dACC_betas-rewardrate_bysubject.png", plot = p.dACC.RR.bysub, 
#        path = figure.path, device = "png", width = 8, height = 6)
# ggsave(filename = "dACC_betas-motiveratings_bysubject.png", plot = p.dACC.motive.bysub, 
#        path = figure.path, device = "png", width = 8, height = 6)
# ggsave(filename = "dACC_RR-motiveratings_bysubject.png", plot = p.dACC.RR.bysub, 
#        path = figure.path, device = "png", width = 8, height = 6)
```


## Selectivity of dACC: Analyzing additional ROIs
```{r}
m.dACC.RR<-lme4::lmer(formula = meanRR ~ mot9Code + value.bundle.mean + (1+mot9Code|subID), 
                      data = data.dACC.sum)
m.vmpfc.RR<-lme4::lmer(formula = meanRR ~ mot9Code + value.bundle.mean + (1+mot9Code|subID), 
                      data = data.vmpfc.sum)
#m.vmpfc.RR.null<-update(m.vmpfc.RR, formula = ~ . -mot9Code -value.bundle.mean )
#BF_vmpfc_RR = BIC(m.vmpfc.RR.null) - BIC(m.vmpfc.RR)
...
# # FDR corrected
# p.adjust(p = c(.05,.06,.66), method = "none", n = 3)
# 
# p.adjust(p = c(.016,.093,.585,.562,.288,.607), method = "none")
# p.adjust(p = c(.016,.093,.585,.562,.288,.607), method = "fdr")
```
...
#brm(formula = meanRR ~ mot9Code + value.bundle.mean + (1+mot9Code|subID), data = data.caudate.sum)
```
...
```{r}
tmp1<-lme4::lmer(formula = value.bundle.mean ~ moneyCode + liqCode + meanRR + (1|subID), 
                data = data.dACC.sum)
tmp2<-lme4::lmer(formula = value.bundle.mean ~ moneyCode + liqCode + meanRR + motive_rating + (1|subID), 
                data = data.dACC.sum)
tab_model(tmp1,tmp2, 
          title= "dACC Betas by Reward Rate and Motivation Ratings",
          dv.labels = c("dACC Betas","dACC Betas"),
          pred.labels = c("Intercept","Money","Liquid","Reward Rate","Motivation Ratings"),
          show.re.var = FALSE, show.stat=TRUE, show.icc = FALSE)
...
tmp2<-lme4::lmer(formula = value.bundle.mean ~ moneyCode + liqCode + meanRR + motive_rating + (1|subID), 
                data = data.putamen.sum)
tab_model(tmp1,tmp2,
          title= "Putamen Betas by Reward Rate and Motivation Ratings",
          dv.labels = c("Putamen Betas","Putamen Betas"),
          pred.labels = c("Intercept","Money","Liquid","Reward Rate","Motivation Ratings"),
          show.re.var = FALSE, show.stat=TRUE, show.icc = FALSE)
...
tmp2<-lme4::lmer(formula = value.bundle.mean ~ moneyCode + liqCode + meanRR + motive_rating + (1|subID), 
                data = data.caudate.sum)
tab_model(tmp1,tmp2,
          title= "Caudate Betas by Reward Rate and Motivation Ratings",
          dv.labels = c("Caudate Betas","Caudate Betas"),
          pred.labels = c("Intercept","Money","Liquid","Reward Rate","Motivation Ratings"),
          show.re.var = FALSE, show.stat=TRUE, show.icc = FALSE)
```

```{r}
tmp1<-lme4::lmer(formula = meanRR ~ moneyCode + liqCode + value.bundle.mean + (1|subID), 
                data = data.dACC.sum)
tmp2<-lme4::lmer(formula = meanRR ~ moneyCode + liqCode + value.bundle.mean + motive_rating + (1|subID), 
                data = data.dACC.sum)
tab_model(tmp1,tmp2)
...
```{r}
cor.test(data.dACC.sum.sub$value.bundle.mean.normed, data.putamen.sum.sub$value.bundle.mean.normed)
plot(data.dACC.sum.sub$value.bundle.mean.normed, data.putamen.sum.sub$value.bundle.mean.normed)

cor.test(data.dACC.sum.sub$value.bundle.mean.normed, data.caudate.sum.sub$value.bundle.mean.normed)
...
corrplot(corr = cor(data.corr), type= "upper", 
         tl.col = "black", order = "hclust", addCoef.col="black", sig.level = 0.05)
